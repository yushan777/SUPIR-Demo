    ...
    with gr.Blocks(title="SUPIR Detailer/Enhancer/Upscaler", theme=custom_theme,  
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.TabItem("2. SUPIR"):
                ...
                | `Upscale` | Upscale factor for the original input image. <br>Default: `2` <br>Upscaling of the input image is actually performed before the denoising and sampling stage. The higher the scale factor, the slower the process. <br>The validation dataset in the original repo suggests that the input image is ideally expected to be 512x512 (or total pixel equivalent). <br>Results usually look better when the input image is sized to this resolution (even if you have a higher res. image available) and upscaled from there. <br> The sweet spot seems to be between 3x and 5x.  Beyond that the quality begins to collapse.|
                | `Skip Denoise Stage` | Skips the VAE Denoiser Stage. Default: `'False'`<br>Bypass the artifact removal preprocessing step that uses the specialized VAE denoise encoder. <br>This usually ends up with the input image slightly softened (if you inspected it at this stage) <br>This is to avoid SUPIR treating low-res/compression artifacts as detail to be enhanced. <br>Without this step you will likely end up with a very over-sharpened, noisy and unnatural look. <br>You may wish to skip this step if you want do do your own pre-processing.|
