    ...
    with gr.Blocks(title="SUPIR Detailer/Enhancer/Upscaler", theme=custom_theme,  
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.TabItem("2. SUPIR"):
                ...
                | `Upscale` | Upscale factor for the original input image. <br>Default: `2` <br>Upscaling of the input image is performed before the denoising and sampling stage. <br>Both dimensions are multiplied by the upscale value. If the smaller of the dimensions is still < 1024px, the image is further enlarged to minimum of 1024px (aspect ratio maintained). This is just to give SDXL a comfortable working resolution.  **Note** that dimensions are snapped to the nearest multiple of 64. <br>The sweet spot seems to be between 1.5x and 2.5x (1024x1024) or 3x and 5x (512x512). Beyond that, the quality begins to collapse. <br>The higher the scale factor, the slower the process. |
                | `Skip Denoise Stage` | Skips the VAE Denoiser Stage. Default: `'False'`<br> Bypass the artifact removal preprocessing step that uses the specialized VAE denoise encoder. This usually ends up with the image slightly softened (if you inspected it at this stage).  This is to avoid SUPIR treating low-res/compression artifacts as detail to be enhanced. <br>You may wish to skip this step if you want do do your own pre-processing OR if your input image is clean and free of low-res/compression artifacts or other degradations.|
