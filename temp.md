...
* `--sampler_mode`
  Sampler choice",  Options: `['TiledRestoreEDMSampler', 'RestoreEDMSampler']`. 
...
* `--upscale`
  Upsampling ratio for the input.  
...
* `--s_churn`
  Adds random noise to encourage variation. Default: `5`
...
* `--control_scale_end` 
  Structural guidance from input image, end strength. Default: `0.9`
...
* `--diff_dtype`  
  ...
  Allows overriding the default precision **independently of model-wide settings**, unless `--loading_half_params` is already set.  
...
* `--sampler_tile_size`
  Tile size for when using `TiledRestoreEDMSampler` sampler.  
  This is the size of each tile that the image is divided into during the tiled sampling process.  
  For example, a `tile_size` of 128 means that the image will be split into tiles of 128x128 pixels.

* `--sampler_tile_stride`
  Tile stride for when using `TiledRestoreEDMSampler` sampler.  
  Controls how much tiles overlap during the sampling process. when using the `TiledRestoreEDMSampler`. 

  A **smaller** tile_stride means **more** overlap between tiles, which helps blend the edges and reduce visible seams, but increases computation.
  A **larger** tile_stride means **less** overlap (or none), which is faster but may cause visible artifacts at the tile boundaries.

  `Overlap = tile_size - tile_stride`

  `Greater overlap ⇨ smaller stride`
  `Less overlap ⇨ larger stride`

  For example, if `tile_size` is 128 and `tile_stride` is 64, then each tile overlaps its neighbors by 64 pixels.
